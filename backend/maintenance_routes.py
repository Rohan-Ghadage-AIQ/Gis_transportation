...
_maint_office = {"latitude": 19.05507294355211, "longitude": 72.87538873375874}
...
async def maintenance_compute(body: MaintenanceComputeRequest):
    ...
    """
    global _maint_tasks_data, _maint_technicians, _maint_office, MAINTENANCE_TEAM_SIZE
    ...
    try:
        ...
        print("=" * 60)
        print(f"Tasks: {len(_maint_tasks_data)}, Technicians: {len(_maint_technicians)}, Team Size: {team_size}")
        ...
        result = await solve_maintenance_vrp(office_lon, office_lat, _maint_technicians, team_size)
        ...
        return JSONResponse(content={
            "status": "success",
            "message": f"Maintenance optimization completed. {result['total_technicians_used']} technicians assigned {result['total_tasks_assigned']} tasks."
        })
...
async def maintenance_results():
    ...
    global _maint_technicians, _maint_office

    try:
        conn = get_db_connection()
        cur = conn.cursor(cursor_factory=RealDictCursor)

        team_size = len(_maint_technicians) if _maint_technicians else MAINTENANCE_TEAM_SIZE
        ...
        for veh in summary["vehicles"]:
            ...
            shift_start = 480
            shift_end = 1080
            if vid - 1 < len(_maint_technicians):
                tech_name = _maint_technicians[vid - 1]['name']
                shift_start = _maint_technicians[vid - 1]['shift_start']
                shift_end = _maint_technicians[vid - 1]['shift_end']
